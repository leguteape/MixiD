...
inline float u16_to_float(uint16_t data) {
    float temp = data & 0x0FFF;
    temp /=  16.0;
...
//Init with enough growth for maximum possible data
inline unsigned char meter_data[128]; //TODO: resize for device
...
void poll_vu() 
{
	int result = libusb_control_transfer(
		devh,
		0xA1,           // bmRequestType
		0x03,           // bRequest (GET_MEM)
		0x0000,         // wValue (Offset)
		0x3c04,         // wIndex (Mixer Unit 0x3C via Interface 4)
		meter_data,     // Buffer
		32,             // wLength
		20              // Timeout (ms)
	);
